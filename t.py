...
try:
    _CUSTOM_CHANNEL_MAPPING = [
        int(channel)
        for channel in _CONFIG.get("audio_processing", "custom_channel_mapping").split(
            ","
        )
    ]
except:
...
try:
    _CUSTOM_CHANNEL_FREQUENCIES = [
        int(channel)
        for channel in _CONFIG.get(
            "audio_processing", "custom_channel_frequencies"
        ).split(",")
    ]
except:
    _CUSTOM_CHANNEL_FREQUENCIES = 0
try:
    _PLAYLIST_PATH = _CONFIG.get("lightshow", "playlist_path").replace(
...
def calculate_channel_frequency(
    min_frequency, max_frequency, custom_channel_mapping, custom_channel_frequencies
):
    ...
    if custom_channel_mapping != 0 and len(custom_channel_mapping) == hc.GPIOLEN:
        logging.debug(
            "Custom Channel Mapping is being used: %s", str(custom_channel_mapping)
        )
...
def main():
    ...
    filegroup.add_argument(
        "--playlist", default=_PLAYLIST_PATH, help="Playlist to choose song from."
    )
    filegroup.add_argument(
        "--file",
        help="path to the song to play (required if no" "playlist is designated)",
    )
    parser.add_argument(
        "--readcache",
        type=int,
    ...
    if args.playlist != None and args.file == None:
        ...
        if most_votes[0] != None:
            ...
            # Play next song in the lineup
                ...
                next_song = (
                    (song_to_play + 1) if ((song_to_play + 1) <= len(songs) - 1) else 0
                )
    ...
    logging.info(
        ...
        + song_filename
        + " ("
        + str(musicfile.getnframes() / sample_rate)
        + " sec)"
    )
    ...
    cache_found = False
    cache_filename = (
        os.path.dirname(song_filename)
        + "/."
        + os.path.basename(song_filename)
        + ".sync.gz"
    )
    ...
    if args.readcache:
        ...
        try:
            ...
            logging.warn(
                "Cached sync data song_filename not found: '"
                + cache_filename
        ...
        for i in range(0, hc.GPIOLEN):
            if hc.is_pin_pwm(i):
                ...
                if limit[i] < matrix[i] * _LIMIT_THRESHOLD:
                    ...
                    logging.debug(
                        "++++ channel: {0}; limit: {1:.3f}".format(i, limit[i])
                    )
